...
export default function PaymentMixKPI() {
  ...
  const getStatusIcon = (variance: number) => {
    ...
    if (Math.abs(variance) <= 5) return '⚠️'
    return '🚨'
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="bg-card p-4 rounded-lg border">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="text-center p-3 bg-green-50 dark:bg-green-900/20 rounded border border-green-200">
            ...
            <div className="text-xs font-normal text-green-600">UPI 50–70%, Cards 20–30%, Cash 5–15%</div>
          </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-2">
              <span className="text-xs font-bold text-black dark:text-white">
                ₹{chainTotalCash.toLocaleString('en-IN')}
              </span>
              <span>💰</span>
            </div>
          </div>
            ...
            <div key={location.restaurant.id} className={`p-3 rounded border ${getStatusBg(location.cashVariance)}`}>
              ...
              <div className="grid grid-cols-4 gap-4 mb-3 text-xs">
                <div className="text-center p-2 bg-blue-50 dark:bg-blue-900/20 rounded">
                  ...
                  <div className="text-gray-500">₹{location.paymentBreakdown.upi.toLocaleString('en-IN')}</div>
                </div>
                <div className="text-center p-2 bg-green-50 dark:bg-green-900/20 rounded">
                  ...
                  <div className="text-gray-500">₹{location.paymentBreakdown.cards.toLocaleString('en-IN')}</div>
                </div>
                <div className="text-center p-2 bg-orange-50 dark:bg-orange-900/20 rounded">
                  ...
                  <div className="text-gray-500">₹{location.paymentBreakdown.cash.toLocaleString('en-IN')}</div>
                </div>
                <div className="text-center p-2 bg-purple-50 dark:bg-purple-900/20 rounded">
                  ...
                  <div className="text-gray-500">₹{location.paymentBreakdown.wallets.toLocaleString('en-IN')}</div>
                </div>
              </div>
              ...
              <div className="mt-3 pt-2 border-t border-gray-200 dark:border-gray-700">
                ...
                <div className="grid grid-cols-4 gap-4 text-xs">
                  <div>
                    ...
                    <div className="font-normal text-black dark:text-white">₹{location.eodReconciliation.expectedCash.toLocaleString('en-IN')}</div>
                  </div>
                  <div>
                    <span className="text-gray-500">Actual Cash:</span>
                    <div className="font-normal text-black dark:text-white">₹{location.eodReconciliation.actualCash.toLocaleString('en-IN')}</div>
                  </div>
                  <div>
                    <span className="text-gray-500">Variance:</span>
                    <div className={`font-normal ${location.eodReconciliation.variance >= 0 ? 'text-green-600' : 'text-red-600'}`}>
                      {location.eodReconciliation.variance >= 0 ? '+' : ''}₹{location.eodReconciliation.variance.toLocaleString('en-IN')}
                    </div>
                  </div>
                  <div>
              ...
              <div className="mt-2 text-xs">
                ...
                <span className="ml-2 font-bold text-green-600">
                  ₹{location.digitalPaymentTotal.toLocaleString('en-IN')} ({((location.digitalPaymentTotal / location.totalRevenue) * 100).toFixed(1)}%)
                </span>
              </div>
            </div>
          ))}
      ...
      <div className="bg-card p-4 rounded-lg border">
        <h2 className="text-xs font-bold text-black dark:text-white mb-4">
          📊 Daily Payment Mix Trend (This Week %)
        </h2>
        <div className="space-y-3">
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-2">
            ...
            <ul className="text-xs font-normal text-gray-500 space-y-1">
              <li>• PetPooja POS /orders (payment_mode field)</li>
              <li>• Bank reconciliation reports</li>
              <li>• Cash register EOD counts</li>
            </ul>
          </div>
          <div className="space-y-2">
            ...
            <ul className="text-xs font-normal text-gray-500 space-y-1">
              <li>• Daily</li>
              <li>• Alert: Red if cash variance >2%</li>
              <li>• Drill down: by payment method, by location, by daypart</li>
            </ul>
          </div>
        </div>
      </div>
      ...
      <div className="bg-orange-50 dark:bg-orange-900/20 p-4 rounded-lg border border-orange-200">
        ...
        <div className="space-y-1">
          <p className="text-xs font-normal text-orange-800 dark:text-orange-300">
            • UPI-dominant in metros (50–70%); cash variance >2% requires investigation
          </p>
          <p className="text-xs font-normal text-orange-800 dark:text-orange-300">
            • Critical for EOD reconciliation and theft prevention
          </p>
          <p className="text-xs font-normal text-orange-800 dark:text-orange-300">
            • Monitor digital payment failures and customer payment preferences
          </p>
        </div>
      </div>
